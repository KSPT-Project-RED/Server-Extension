...
public class ZoneJoinEventHandler extends BaseServerEventHandler {
    ...
    public void handleServerEvent(ISFSEvent event) throws SFSException {
        trace("ZoneJoinEventHandler");
        User theUser = (User) event.getParameter(SFSEventParam.USER);

        // dbid is a hidden UserVariable, available only server side
        UserVariable dbId = new SFSUserVariable("dbid", theUser.getSession().getProperty(DominionExtension.DATABASE_ID));
        dbId.setHidden(true);

        // The avatar UserVariable is a regular UserVariable
        UserVariable avatar = new SFSUserVariable("avatar", "avatar_" + theUser.getName() + ".jpg");

        // Set the variables
        List<UserVariable> vars = Arrays.asList(dbId, avatar);
        ...
        if (lobby == null) {
            throw new SFSException("The Lobby Room was not found! Make sure a Room called 'The Lobby' exists in " +
                    "the Zone to make this example work correctly.");
        }
